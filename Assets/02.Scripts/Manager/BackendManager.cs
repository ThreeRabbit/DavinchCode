...
public class BackendManager : TRSingleton<BackendManager>
{
    ...
    public void FederationLogin(FederationType federationType, string token, UnityAction success = null, UnityAction fail = null)
    {
        ...
        SendQueue.Enqueue(Backend.BMember.AuthorizeFederation, token, federationType, (callback) =>
        {
            if(callback.IsSuccess())
			{
                BackendLog(callback, LogType.GREEN, $"BackendManager: {federationType.ToString()} Login Success");
                success?.Invoke();
            }
			else 
            {
                BackendLog(callback, LogType.RED, $"BackendManager: {federationType.ToString()} Login Fail");
            }
        });
    }
